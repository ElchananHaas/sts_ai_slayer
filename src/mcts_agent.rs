...
use crate::{game::{ChoiceState, Game}, rng::Rng};
...
fn take_indexed_action<'a>(state: ChoiceState<'a>, idx: usize, verbose: bool) -> ChoiceState<'a> {
    match state {
        ...
        ChoiceState::ChooseEnemyState(choose_enemy_state) => {
            let actions: Vec<crate::game::ChooseEnemyAction> = choose_enemy_state.available_actions();
            if verbose {
...
fn mcts_rollout(mut state: ChoiceState, value_map: &mut HashMap<u64, MctsEntry>) -> f32 { 
    ...
    let reward: i32 = loop {
        ...
        let num_actions= match &state {
            ChoiceState::PlayCardState(play_card_state) => {play_card_state.available_actions().len()},
            ChoiceState::ChooseEnemyState(choose_enemy_state) => {choose_enemy_state.available_actions().len()},
            ChoiceState::WinState(game) => {break game.floor;},
            ChoiceState::LossState(game) => {break game.floor;},
    ...
    for i in 0..state_hashes.len() {
        value_map.get_mut(&state_hashes[i]).expect("State found").update(taken_actions[i], reward);
    }
    reward as f32
}
